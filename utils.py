...
from celeba_dataset import CelebAGender
...
def get_dataset(dataset, data_path):
    if dataset == 'MNIST':
        ...
        dst_train = CelebAGender(data_path, split='train', transform=transform, download=True)
        dst_test = CelebAGender(data_path, split='test', transform=transform, download=True)
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args, return_loss=False):
    ...
    if return_loss:
        return net, acc_train, acc_test, loss_test
    else:
...
def get_eval_pool(eval_mode, model, model_eval):
    if eval_mode == 'M': # multiple architectures
        ...
        model_eval_pool = ['MLP', 'ConvNet', 'ConvNetIN', 'ConvNetGN', 'ConvNetBN', 'LeNet', 'AlexNet', 'VGG11', 'ResNet18']
...
## strong matching loss
def pdist(X, Y):
    normX = torch.diag(torch.matmul(X, torch.t(X))).unsqueeze(1)
    normY = torch.diag(torch.matmul(Y, torch.t(Y))).unsqueeze(0)
    XY = torch.matmul(X, torch.t(Y))
    return normX.repeat(1, Y.size(0)) - 2 * XY + normY.repeat(X.size(0), 1)

def estimated_em_loss(output_syn, output_real):
    dist = pdist(output_real, output_syn)
    prob = F.softmax(dist, dim=1).detach() ## disable grad
    return torch.mean(torch.sum(prob * dist, dim=1))

def cos_loss(mean_output_syn, mean_output_real):
    dot_product = torch.sum(mean_output_syn * mean_output_real)
    norm_product = max((torch.norm(mean_output_syn) * \
                    torch.norm(mean_output_real)).item(), 1e-8)
    return dot_product/norm_product

def poisson_sampling(num_samples, sample_rate):
    indices = np.random.rand(num_samples) < sample_rate
    return indices